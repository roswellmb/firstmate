...
When a \`needs-decision\` you are escalating IS a choice between options, raise it with its fields separate so the question, the options and the recommendation reach the captain intact instead of as one blob - this hop is where a decision raised by one of your own crewmates would otherwise be flattened back into prose:
   \`$FM_ROOT/bin/fm-decision-raise.sh raise --status $STATUS_FILE --key {slug} --question "{one sentence}" --option "{label}" --option "{label}" --consequence {id} "{what follows if this one is picked}" --consequence {id} "{what follows if this one is picked}" --recommend {id} --because "{why}"\`
...
Give each option its own \`--consequence\`: a recommendation is only honest when the captain could have disagreed with it, and labels he cannot weigh against each other leave him taking the marked one because it is the only argued one.
It is optional and it is not all-or-nothing - arguing some options and not others is recorded and reported as such - but a consequence that just repeats the label, the question, the rationale or another option is refused, because that is padding rather than reasoning.
...
4. Report status by appending one line:
   \`echo "{state}: {one short line}" >> $STATUS_FILE\`
   States: working, needs-decision, blocked, $PAUSED_VERB, done, failed.
   Most lines need no key. When a line carries one, it sits BEFORE the colon, in the state prefix:
   \`echo "{state} [key={slug}]: {one short line}" >> $STATUS_FILE\`
   A key written after the colon is silently read as unkeyed, so nothing later can close it by that name.
