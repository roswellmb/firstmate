...
with_remote_route_locks() { # <secondmate-id> <function> <args...>
  ...
  fm_lock_acquire_wait "$ACTIVE_REGISTRY_LOCK" || { release_remote_locks; return 1; }
  ...
  fm_lock_acquire_wait "$ACTIVE_HANDOFF_LOCK" || { release_remote_locks; return 1; }
...
ACTIVE_REGISTRY_LOCK=$(secondmate_registry_lock_path "$STATE")
fm_lock_acquire_wait "$ACTIVE_REGISTRY_LOCK" || { release_remote_locks; exit 1; }
REMOTE=$(secondmate_registry_field "$REG" "$ID" remote 2>/dev/null || true)
if [ "$REMOTE" = 1 ]; then
  ACTIVE_HANDOFF_LOCK="$STATE/.backlog-handoff-$ID.lock"
  fm_lock_acquire_wait "$ACTIVE_HANDOFF_LOCK" || { release_remote_locks; exit 1; }
