...
FM_DISPATCH_POOL_ROOT=  # path whose filesystem hosts isolated working copies; default $HOME/.treehouse when present, else $FM_HOME
...
FM_DISPATCH_BUDGET_SECS=20  # 1..30-second bound per external call inside a dispatch scan; anything else is reported as a budget-invalid verdict, and hitting the bound is reported rather than silently killed
...
FM_DISPATCH_COPY_MEASURE_SECS=21600  # cache lifetime for the largest-isolated-copy measurement in the dispatch capacity floor
