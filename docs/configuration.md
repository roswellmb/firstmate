...
FM_WATCH_LOCK_WAIT_TIMEOUT=30   # seconds the WATCHER waits on its own locks (the wake queue and the recovery marker) before degrading; empty, zero, or non-numeric uses 30. Those locks are held for sub-millisecond file operations, so a tight budget is correct and is what keeps the liveness beacon fresh while the watcher degrades. Constraint: 2*(this+1) + cycle tail + FM_CHECK_TIMEOUT*N < FM_GUARD_GRACE, where the beacon is touched only at the top of the poll loop, the tail is FM_INACTIVE_RECONCILE_BUDGET_SECS (10) plus FM_POLL (15), and N is the number of registered state/*.check.sh. At stock defaults that is 62+25+30*N, so N up to 7. N is UNBOUNDED and this knob cannot fix that: with enough armed checks the check sweep alone ages the beacon past the grace whatever this is set to
