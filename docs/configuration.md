...
`data/` holds durable private fleet records such as the project and secondmate registries, captain preferences, optional shared captain preferences, learnings, backlog, briefs, scout reports, and the per-task record of any status-log decision that was still open when teardown removed its status log (`bin/fm-teardown.sh`).
...
FM_DISPATCH_POOL_ROOT=  # pool root, used twice: its filesystem is measured for free space, and its contents are walked for the largest isolated working copy (grandchild depth, since a pool-root child is a per-repo pool, falling back to the child when it holds no subdirectory); default $HOME/.treehouse when present, else $FM_HOME, which is a filesystem handle only and is never walked
FM_DISPATCH_OS_RESERVE_MB=  # operating-system disk reserve in MiB; default installed physical RAM (see bin/fm-dispatch-poll.sh)
FM_DISPATCH_BUDGET_SECS=20  # 1..30-second bound per bounded unit of a dispatch scan (the tasks-axi ready read, the tasks-axi list cross-check, and the whole copy measurement under one shared deadline); anything else is reported as a budget-invalid verdict, and hitting the bound is reported rather than silently killed
FM_DISPATCH_RESURFACE_SECS=21600  # bounded re-surface for an unchanged blocked or unknown dispatch verdict; never applies to a ready one
FM_DISPATCH_COPY_MEASURE_SECS=21600  # cache lifetime for the largest-isolated-copy measurement behind the dispatch capacity floor and its one-copy deadband
FM_DISPATCH_LIST_LIMIT=12  # identifiers named per group in a dispatch wake before the disclosed "+N more"; the gain that decides a ready wake is still computed over the whole briefed set
FM_CHECK_INTERVAL=300   # seconds between slow checks (the fleet-level dispatch-readiness scan that opens each sweep under its own FM_DISPATCH_* bounds, authenticated merge polls, custom checks, or Relay dispatch)
...
FM_CREW_STATE_RUNS_LIMIT=200  # recent no-mistakes run rows scanned to locate this branch's own newest run row, only when axi status answered for a different branch; that row still has to bind to the current code, and a branch whose newest row sits past this cap reads as no run at all
...
FM_LOCK_WAIT_TIMEOUT=300   # seconds a blocking lock wait keeps retrying a live holder before giving up with a diagnostic naming the lock and holder; empty, zero, or non-numeric uses 300. This is the ceiling for locks with deliberate long holds. LOWER bound ~120s = FM_STARTUP_NETWORK_TIMEOUT, the longest of them (the deferred network stage keeps the fleet lock for its whole budget); at or under it a session on the start path falls back to read-only. Not coupled to FM_GUARD_GRACE: the watcher applies FM_WATCH_LOCK_WAIT_TIMEOUT to its own process, so the beacon-freshness constraint lives there
FM_WATCH_LOCK_WAIT_TIMEOUT=30   # seconds the WATCHER PROCESS waits on any lock before degrading; empty, zero, or non-numeric uses 30. bin/fm-watch.sh puts this in a private, never-exported variable that fm_lock_acquire_wait reads in preference to FM_LOCK_WAIT_TIMEOUT, so it covers every lock that process waits on - including the ones taken inside fm_wake_append and inside libraries sourced into the watcher shell - without disturbing FM_LOCK_WAIT_TIMEOUT itself. The watcher's SUBPROCESSES (registered *.check.sh, fm-pr-poll, fm-procevent, fm-inactive-reconcile, fm-dispatch-poll, fm-crew-state) therefore still see whatever FM_LOCK_WAIT_TIMEOUT the watcher itself inherited, so exporting a tuned global reaches them unchanged. Constraint: 2*(this+1) + cycle tail + FM_CHECK_TIMEOUT*N < FM_GUARD_GRACE, where the beacon is touched only at the top of the poll loop, the tail is FM_INACTIVE_RECONCILE_BUDGET_SECS (10) plus FM_POLL (15), and N is the number of registered state/*.check.sh. N is UNBOUNDED and this knob cannot fix that: with enough armed checks the check sweep alone ages the beacon past the grace whatever this is set to
FM_GUARD_GRACE=300      # seconds before guard warnings, arm health checks, and the primary turn-end guard treat a watcher beacon as stale. This is the UPPER bound on FM_WATCH_LOCK_WAIT_TIMEOUT (see the constraint recorded there): lowering this without lowering that one makes a watcher waiting out a wedged lock look dead, and the re-arm path then tells the operator to stop it
