...
FM_LOCK_WAIT_TIMEOUT=300   # seconds a blocking lock wait keeps retrying a live holder before giving up with a diagnostic naming the lock and holder; empty, zero, or non-numeric uses 300. This is the DEFAULT ceiling, used by every caller that does not pass its own; it is sized for the locks with deliberate long holds. LOWER bound ~120s = FM_STARTUP_NETWORK_TIMEOUT, the longest of them (the deferred network stage keeps the fleet lock for its whole budget); at or under it a session on the start path falls back to read-only. Not coupled to FM_GUARD_GRACE: the watcher uses FM_WATCH_LOCK_WAIT_TIMEOUT for its own locks, so the beacon-freshness constraint lives there
FM_WATCH_LOCK_WAIT_TIMEOUT=30   # seconds the WATCHER PROCESS waits on any lock before degrading; empty, zero, or non-numeric uses 30. bin/fm-watch.sh assigns this to its own shell's FM_LOCK_WAIT_TIMEOUT at startup, unexported, so it covers every lock that process waits on - including the ones taken inside fm_wake_append and inside libraries sourced into the watcher shell - while its SUBPROCESSES (registered *.check.sh, fm-pr-poll, fm-procevent, fm-inactive-reconcile, fm-dispatch-poll, fm-crew-state) keep the generous global and are unchanged from previous behaviour. Constraint: 2*(this+1) + cycle tail + FM_CHECK_TIMEOUT*N < FM_GUARD_GRACE, where the beacon is touched only at the top of the poll loop, the tail is FM_INACTIVE_RECONCILE_BUDGET_SECS (10) plus FM_POLL (15), and N is the number of registered state/*.check.sh. N is UNBOUNDED and this knob cannot fix that: with enough armed checks the check sweep alone ages the beacon past the grace whatever this is set to
