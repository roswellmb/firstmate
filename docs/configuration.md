...
FM_LOCK_WAIT_TIMEOUT=120   # seconds a blocking lock wait keeps retrying a live holder before giving up with a diagnostic naming the lock and holder; empty, zero, or non-numeric uses 120. MUST stay below half of FM_GUARD_GRACE: one watcher poll cycle can contain two contended waits and the beacon is touched only between cycles, so a wedged lock ages it by about 2*(this+1) and anything above half the grace makes a watcher that is degrading exactly as designed report as stale
FM_GUARD_GRACE=300      # seconds before guard warnings, arm health checks, and the primary turn-end guard treat a watcher beacon as stale. Keep it above twice FM_LOCK_WAIT_TIMEOUT: lowering this without lowering that one makes a watcher waiting out a wedged lock look dead, and the re-arm path then tells the operator to stop it
