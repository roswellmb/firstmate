...
FM_LOCK_WAIT_TIMEOUT=140   # seconds a blocking lock wait keeps retrying a live holder before giving up with a diagnostic naming the lock and holder; empty, zero, or non-numeric uses 140. Pinned inside a narrow window by two unrelated knobs, so do not tune it alone. LOWER bound ~120s = FM_STARTUP_NETWORK_TIMEOUT, the longest deliberate hold (the deferred network stage keeps the fleet lock for its whole budget); at or under it a session on the start path falls back to read-only. UPPER bound ~149s = half of FM_GUARD_GRACE, because a watcher poll cycle can contain two contended waits and the beacon is touched only between cycles, so a wedged lock ages it by about 2*(this+1) and anything higher makes a watcher that is degrading exactly as designed report as stale. 140 is mid-window on purpose
FM_GUARD_GRACE=300      # seconds before guard warnings, arm health checks, and the primary turn-end guard treat a watcher beacon as stale. This is the UPPER bound on FM_LOCK_WAIT_TIMEOUT, which must stay below half of it: lowering this without lowering that one makes a watcher waiting out a wedged lock look dead, and the re-arm path then tells the operator to stop it
