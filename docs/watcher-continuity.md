...
The same suite covers the abandonment and contention paths of the blocking lock wait: a watcher signalled inside its own recovery-marker lock still exits and releases both that lock and the singleton, a contended wake-queue lock skips one cycle and leaves the watcher alive while an unsafe recovery marker still exits it loudly, and both the watcher's direct waits and the ones inside `fm_wake_append` give up on `FM_WATCH_LOCK_WAIT_TIMEOUT` rather than the global ceiling.
