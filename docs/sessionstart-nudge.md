...
One stage does carry a bound of its own: the fleet-level dispatch-readiness scan runs under a short backstop derived from this same budget, so the parent passes its already-normalized budget down to the child and no stage can be given a bound its enclosing budget cannot honour, and a scan the backstop cuts short is reported in the digest instead of being swallowed.
