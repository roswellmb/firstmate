...
Because cleanup deletes the status log, `bin/fm-teardown.sh` refuses while a keyed decision is still open and names that closer, and a forced teardown records what it discarded in `data/<task-id>/discarded-decisions.md`, written at the end of each destroying path's refusals and immediately before the status log is removed, so no open decision leaves the record without something true being written about why.
