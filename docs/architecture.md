...
Because cleanup deletes the status log, `bin/fm-teardown.sh` refuses while a keyed decision is still open and names that closer, and a forced teardown records what was open in `data/<task-id>/discarded-decisions.md` before it destroys anything, as an authorization to discard rather than a claim that the teardown finished, so no open decision leaves the record without something TRUE being written about why.
