...
`bin/fm-decision-lib.sh`'s header owns the record format, the (task id, decision key, option id) option identity that an answering surface must agree with rather than inventing its own, and the single degeneracy check the writer and every reader both call.
...
[Captain project marks](configuration.md#captain-project-marks-configproject-marks) owns the file, its two kinds, and where the refusal is enforced.
