...
The watcher's slow-check sweep opens with `bin/fm-dispatch-poll.sh`, which turns the backlog from something firstmate must remember to look at into something that wakes it: it notices when briefed queued work becomes dispatchable and surfaces one `check: dispatch` wake, and locked session start runs the identical scan so the verdict lands in the startup digest instead of arriving separately.
Noticing is all it does - it never spawns, never ranks, and never speaks unless something actionable appeared, because deciding to dispatch needs the project, delivery-mode, autonomy, and brief judgement that AGENTS.md sections 7 and 11 keep with firstmate.
That script's header owns its verdicts, what counts as actionable, the durable record behind that judgement, its no-repeat rule, why it needs no `state/<id>.check.sh` identity or `bin/fm-check-register.sh` binding, and where its capacity numbers come from.
...
A decision that is a choice between options can instead be raised through `bin/fm-decision-raise.sh`, which appends that same unchanged status line and records the question, the options, and the recommendation as separate fields in an append-only sidecar at `state/<id>.decisions` joined to the folded decision by (task id, decision key), so the fields survive every hop while the open/resolved grammar keeps its single owner in `fm-classify-lib.sh` and every existing status-log reader keeps working byte for byte.
Structure is available rather than compulsory, so a decision with no such record is an ordinary free-text decision and folds, renders, and closes exactly as before.
`fm-wake-drain.sh` renders those fields under their folded entry and, when its byte cap forces a cut, says how many decisions lost their fields and where to read them so a bounded view never reads as a complete one, and prints a MALFORMED banner with the fields withheld when the record is degenerate, so a decision that arrived without a real question looks wrong at the relay instead of reaching the captain looking structured.
Each option may additionally carry its own consequence - what follows if the captain picks that one - attached by option id so the field is optional per option and a decision that omits it entirely is byte-identical to one raised before the field existed.
Coverage is published as a count rather than enforced, because refusing a partially argued record would make a worker holding one real consequence invent the rest, so partial coverage is recorded, rendered in full, and reported as "n of m" at the relay while a consequence that repeats its option's label, the question, the rationale or another option's consequence is refused as padding.
`bin/fm-decision-lib.sh`'s header owns the record format, the (task id, decision key, option id) option identity that an answering surface must agree with rather than inventing its own, the deliberate absence of any quality score over a consequence, and the single degeneracy check the writer and every reader both call.
...
That path couples closure to a confirmed submit, so when the worker's endpoint is dead, missing, or already torn down it has nothing to deliver to and cannot close anything; `bin/fm-status-decision-close.sh` is the second closer for exactly that case, writing the answer given elsewhere or an explicit statement that the decision is moot, and proving through the same fold that its line takes effect before appending it.
Because cleanup deletes the status log, `bin/fm-teardown.sh` refuses while a keyed decision is still open and names that closer, and any teardown that reaches the removal with a decision still open records it in `data/<task-id>/discarded-decisions.md`, written at the end of each destroying path's refusals and immediately before the status log is removed - a forced teardown under wording that names the discard authority it carried, an unforced one (the crew can append after the gate has passed) under wording that claims none - so no open decision leaves the record without something true being written about why.
...
Only the branch's current run is ever a candidate, and a run that does not bind to the code in front of the reader is treated as no run at all - it never yields a terminal verdict of its own, and attribution never reaches past it to a superseded run that happens to match - because `failed` and `cancelled` are the words a supervisor acts on by hand-pushing around a pipeline that may still be live.
The crew's own task-keyed sources still answer in that case, so a genuinely finished run whose head diverged is still reported from the crew's own status-log append rather than going silent.
...
`fm-spawn.sh` also owns the base-freshness boundary for every fresh ship and scout into a project that has an origin: no worker starts until its clean task worktree matches the fetched tip of origin's resolved default branch, and any unsafe or unverifiable base stops the spawn.
A project with no origin remote at all, the normal shape of a local-only project, has no such tip to be stale against, so it launches from its current local base instead.
...
Independently, `fm-spawn.sh`, `fm-send.sh`, `fm-control.sh`, `fm-teardown.sh`, and `fm-status-decision-close.sh` source `bin/fm-gate-refuse-lib.sh` and exit with status 3 before fleet mutation when the gate environment marker is present or the current checkout matches the default no-mistakes gate-repository topology.
