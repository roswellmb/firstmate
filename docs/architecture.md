...
Only the branch's current run is ever a candidate, and a run that exists but does not bind to the code in front of the reader resolves to `unknown` rather than to any terminal verdict, because `failed` and `cancelled` are the words a supervisor acts on by hand-pushing around a pipeline that may still be live.
