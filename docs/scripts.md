...
| `fm-decision-raise.sh`   | Raise one decision as separate fields (question, options, per-option consequences, recommendation) and read one back |
| `fm-status-decision-close.sh` | Close one open status-log decision deliberately, recording the answer given elsewhere or that it is moot, when no live worker can be answered |
...
| `fm-project-mode.sh`     | Resolve a project's registered posture: its delivery mode from `data/projects.md`, and with `--mark` the captain project mark recorded for it in `config/project-marks` |
...
| `fm-dispatch-poll.sh` | Notice that briefed queued work has become dispatchable and wake firstmate; never spawns, ranks, or repeats |
...
| `fm-home-currency-lib.sh` | Single owner of the read-only "is this firstmate home at its origin default-branch commit?" comparison |
...
| `fm-decision-lib.sh`     | Shared structured-decision record contract: wire format, the (task, key, option id) option identity, per-option consequence coverage, and the one degeneracy check writer and readers share |
