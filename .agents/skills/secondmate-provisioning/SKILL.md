...
Inherited `config/project-marks` is what makes a captain's project exclusion a fleet control rather than a rule binding only the home that set it: a secondmate home is exactly where an unwatched dispatch happens, so every home enforces the primary's marks from its own inherited copy.
