...
config/github-hosts  optional GitHub Enterprise Server hostnames, one per line; LOCAL, gitignored; docs/configuration.md owns the schema
...
state/               runtime records and signals; gitignored
  ...
  .dispatch-poll .dispatch-copy-reserve .dispatch-poll.lock  the dispatch-readiness verdict record, the cached isolated-copy measurement behind its capacity floor, and the non-blocking single-scan lock; written only by bin/fm-dispatch-poll.sh, the first two safe to delete (forces one re-surface and one remeasure)
...
3. For `check:`, act on the named poll result, including merges, Relay events, process-to-event source results, and `check: dispatch` dispatch-readiness verdicts.
   A dispatch-readiness verdict reports only that briefed work has newly become dispatchable, that dispatch has become impossible for want of machine room, or that the answer stopped being establishable; the intake, delivery-mode, autonomy, and brief judgement in sections 7 and 11 remains entirely yours.
