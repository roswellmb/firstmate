...
state/               runtime records and signals; gitignored
  ...
  .dispatch-poll .dispatch-copy-reserve .dispatch-poll.lock  last surfaced dispatch-readiness verdict, the cached isolated-copy measurement behind its capacity floor, and the non-blocking single-scan lock; written only by bin/fm-dispatch-poll.sh, the first two safe to delete (forces one re-surface and one remeasure)
